...
async def on_ready():
    ...
    log.info("CODE VERSION: review-only-on-buy-v2")
    try:
    ...
    try:
        await bot.tree.sync()  # global sync (μπορεί να αργήσει έως 1 ώρα να εμφανιστεί παντού)
        for guild in bot.guilds:
            bot.tree.copy_global_to(guild=guild)
            await bot.tree.sync(guild=guild)  # instant sync σε κάθε server που είναι ήδη μέσα το bot
...
async def create_ticket_channel(interaction: discord.Interaction, ticket_type: str, emoji: str):
    try:
        ...
        category_name = SUPPORT_CATEGORY_NAME if ticket_type == "support" else BUY_CATEGORY_NAME
        category = discord.utils.get(guild.categories, name=category_name)
        if not category:
            log.info(f"Category '{category_name}' not found, creating it")
            category = await guild.create_category(category_name)
        else:
            log.info(f"Using existing category '{category_name}' (id={category.id})")
...
class CloseTicketView(discord.ui.View):
    ...
    async def close_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            is_buy_ticket = channel.name.lower().startswith("buy-")

            await interaction.response.send_message("Κλείνει το ticket...")

            if opener_id and is_buy_ticket:
                embed = discord.Embed(
...
@bot.tree.command(name="rules", description="Στέλνει τους κανόνες του server")
async def rules(interaction: discord.Interaction):
    try:
        embed = discord.Embed(
            title="📜 Server Rules",
            description=(
                "**Welcome to NexaServices Market!**\n"
                "Please read and follow these rules to keep our community "
        ...
        embed.add_field(
            ...
            value=(
                "‣ No spamming messages, links, or emojis\n"
                "‣ No advertising other servers or services without permission\n"
        ...
        embed.add_field(
            name="3️⃣ Trading & Orders",
            value=(
                "‣ All purchases go through a ticket — never trade outside the server\n"
                "‣ Scamming results in an immediate, permanent ban\n"
                "‣ Double-check details with staff before paying"
            ),
            inline=False
        )
        embed.add_field(name="\u200b", value="━━━━━━━━━━━━━━━━━━", inline=False)
        ...
        embed.add_field(
            name="4️⃣ No NSFW or Illegal Content",
            value=(
                "‣ No NSFW, gore, or shocking content\n"
                "‣ No sharing of illegal content or services"
            ),
            inline=False
        )
        embed.add_field(name="\u200b", value="━━━━━━━━━━━━━━━━━━", inline=False)
        ...
        embed.add_field(
            name="5️⃣ Follow Staff Instructions",
            value=(
                "‣ Staff decisions are final\n"
                "‣ Open a ticket if you have questions or concerns\n"
                "‣ Breaking the rules may result in a warning, mute, kick, or ban"
            ),
            inline=False
        )
        ...
        log.error(f"Rules command failed: {e}")


@bot.tree.command(name="server-setup", description="Στέλνει τα πακέτα Server Setup (Starter/Pro/Elite)")
async def server_setup(interaction: discord.Interaction):
    try:
        ...
        log.error(f"Server-setup command failed: {e}")


@bot.tree.command(name="services", description="Στέλνει τη γενική λίστα υπηρεσιών")
async def services(interaction: discord.Interaction):
    try:
        embed = discord.Embed(
            title="💼 What We Offer",
            description=(
                "**Your one-stop shop for everything Discord.**\n"
                "✨ *Fast • Affordable • High Quality* ✨"
            ),
            color=discord.Color.blue()
        )
        ...
        embed.add_field(
            name="🖥️ Server Setups",
            value=(
                "‣ Full custom server builds, from a blank slate to a polished, "
                "ready-to-use community\n"
                "‣ Categories, channels, permissions & roles tailored to you"
            ),
            inline=False
        )
        embed.add_field(name="\u200b", value="━━━━━━━━━━━━━━━━━━", inline=False)
        ...
        embed.add_field(
            name="🤖 Custom Bot Systems (Built From 0)",
            value=(
                "‣ Fully custom Discord bots, coded from scratch to your exact needs\n"
                "‣ Tickets, moderation, economy, leveling, and more\n"
                "‣ Hosted 24/7 with ongoing support"
            ),
            inline=False
        )
        embed.add_field(name="\u200b", value="━━━━━━━━━━━━━━━━━━", inline=False)
        ...
        embed.add_field(
            name="📅 Subscriptions",
            value=(
                "‣ Flexible monthly plans for ongoing bot maintenance & updates\n"
...
@bot.tree.command(name="terms", description="Στέλνει τους Όρους Χρήσης (Terms of Service)")
async def terms(interaction: discord.Interaction):
    try:
        embed = discord.Embed(
            title="📄 Terms of Service – NexaServices Market",
            description=(
                "**By using NexaServices Market, you agree to the following terms:**\n\n"
                "‣ All purchases are final.\n"
                "‣ No refunds or replacements will be provided after a purchase is completed.\n"
                "‣ Refunds or replacements are only possible if the seller personally agrees.\n"
                "‣ Buyers are responsible for checking product/service details before purchasing.\n"
                "‣ Scamming, chargebacks, fraud, or abusing the platform will result in a permanent ban.\n"
                "‣ Sellers are responsible for the products/services they provide.\n"
                "‣ NexaServices Market is not responsible for deals made between users.\n"
                "‣ By purchasing anything on the platform, you automatically accept these Terms of Service."
            ),
            color=discord.Color.blue()
        )
        embed.set_image(url=BANNER_URL)
        embed.set_footer(text="NexaServices Market © 2026 – All Rights Reserved")
        await interaction.channel.send(embed=embed)
        await interaction.response.send_message("Έτοιμο!", ephemeral=True)
    except Exception as e:
        log.error(f"Terms command failed: {e}")
...
async def setup_tickets(interaction: discord.Interaction):
    try:
        support_embed = discord.Embed(
            ...
            description=(
                "If you need help, have a problem, need a replacement,\n"
                "or have won a reward — click below to open a Support\n"
                "ticket and our staff will assist you shortly.\n\n"
                "━━━━━━━━━━━━━━━━━━"
            ),
            color=discord.Color.blue()
        )
        ...
        buy_embed = discord.Embed(
            title="🛒 Buy Ticket",
            description=(
                "Looking to make a purchase? Open a Buy ticket and\n"
                "our team will guide you through your order."
            ),
            color=discord.Color.blue()
        )
        ...
        banner_embed.set_footer(text="NexaServices | All rights reserved")
